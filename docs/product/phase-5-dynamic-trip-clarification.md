...
- 用户只说“帮我制定北京五日游”，缺少同行人、节奏、兴趣、预算或饮食限制等会影响路线质量的信息。
...
- 一次只展示一个问题。
- 支持单选、多选、文本输入和 `Other` 输入；不提供抵达、离开或日期选择。
- 支持 `Back` 返回上一题并修改。
- 支持 `Next` 前进。
- 必填题未完成时不能进入下一步。
- 选项文案必须短、清晰、可扫描。
...
系统永不主动询问抵达、离开或日期时间；用户在原始问题中主动提供的时间仅作为最终行程生成参考。
...
- 单选题：点击选项后立即显示选中态，锁定当前卡片，并在 350ms 后进入下一题或直接生成。
...
| `days` | 行程天数 | 高优先级 |
| `arrivalTime` | 用户原始问题主动提供的抵达时间，仅供最终生成参考 | 可选 |
| `departureTime` | 用户原始问题主动提供的离开时间，仅供最终生成参考 | 可选 |
| `travelers` | 同行人类型 | 可选 |
| `pace` | 行程节奏 | 可选 |
| `budget` | 预算偏好 | 可选 |
| `interests` | 兴趣偏好 | 可选 |
...
- 每轮动态生成 0 至 5 个问题；信息不足时通常为 3 至 5 个，信息充分时可直接生成。
- 固定优先级为目的地/城市、天数、同行人、行程节奏、兴趣偏好；仅在仍有额度且确实影响计划时补充预算、饮食、住宿区域、避开内容或无障碍需求。
- 问题必须与用户原始请求直接相关。
- 不重复询问用户已经明确给出的信息。
- 不询问与行程生成无关的个人隐私。
- 不生成前端不支持的题型。
...
兜底问题不得重复询问已知字段：优先补齐目的地/城市、天数、同行人、节奏和兴趣；不得询问抵达、离开、日期或时间。
