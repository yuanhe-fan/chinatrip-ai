  ...
  return [
    {
      ...
      content: [
        ...
        "Question priority: destination/cities, days, travelers, pace, then interests. Only if capacity remains and it materially changes the plan, ask about budget, dietary needs, hotel/start area, avoidances, or accessibility.",
        "Never ask for arrival time, departure time, travel dates, or any other date/time selection. Time details voluntarily present in the original user message remain available in knownContext, but must not be collected by this flow.",
        ...
        "Use only these question types: single_choice, multi_choice, text.",
        "Create zero to five questions. Do not ask questions just to reach a count; when information is sparse, ask three to five high-value questions. Keep option labels short and provide allowOther when a predefined choice may be insufficient.",
        ...
        "",
        "Required JSON shape:",
        '{ "intent": "itinerary_planning", "needsClarification": boolean, "reason": string, "extractedContext": { "destination"?: string, "days"?: number, "arrivalTime"?: string, "departureTime"?: string, "travelers"?: string, "pace"?: string, "budget"?: string, "interests"?: string[], "dietaryNeeds"?: string[], "startArea"?: string, "avoidances"?: string[], "specialNeeds"?: string[], "notes"?: string }, "questions": [{ "id": string, "title": string, "description"?: string, "type": "single_choice" | "multi_choice" | "text", "required": boolean, "options"?: [{ "label": string, "value": string }], "allowOther"?: boolean }] }',
      ]
        .filter(Boolean)
        .join("\n"),
    },
